...
3. **EBITDA Margin Risk ($+1\text{ pt}$):** Assigned when EBITDA margin declines $YoY$ ($\text{Margin}_t < \text{Margin}_{t-1}$).
...
│   ├── telecom_master.csv             # Raw multi-year dataset
│   ├── telecom_analytical.csv         # Structured financial indicators
│   └── telecom_overall_ranking.csv    # Aggregated risk rankings
│
├── sql/                               # Database & scoring logic
│   ├── 01_create_tables.sql           # Schema definition
│   ├── 02_risk_signals.sql            # LAG() window functions & risk flags
│   └── 03_risk_scores.sql             # Composite scoring & categorization
│
├── python/                            # Analysis & chart scripts
│   ├── cleaning.py                    # Data parsing & null handling
│   ├── combine_data.py                # Dataset merging script
│   ├── backtest.py                    # T vs T+1 predictive analysis
│   └── plot_risk_charts.py            # Matplotlib figure generators
│
├── charts/                            # Output visualisations
│   ├── risk_trend.png                 # Multi-year risk trajectory chart
│   ├── financial_trends_small_multiples.png
│   └── red_flag_events.png            # Annotated event timeline
│
...
```bash
git clone https://github.com/your-username/telecom-risk-scorecard.git
cd telecom-risk-scorecard
```
...
SOURCE sql/03_risk_scores.sql;
```

### 4. Run Analysis & Generate Charts
```bash
python python/backtest.py
python python/plot_risk_charts.py
```
